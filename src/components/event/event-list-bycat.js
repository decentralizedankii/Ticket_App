...
class EventListByCat extends Component {
  constructor(props) {
    super(props);
    this.state = {
      city_name: '',
  ...
  componentWillMount() {
     this.setState({category_id:this.props.match.params.id},function() {
        var data = {
            page: 1,
  ...
  componentWillReceiveProps(newProps)
  {
      ...
      var data = {
        page: 1,
        category_id:newProps.match.params.id
      }
  ...
  getEvents(data) {
    ...
    axios.post(apiUrl() + "event-list-bycategory", data).then((res) => {
  
        this.setState({ total: res.data.total_event[0].total_event });
        console.log(res.data.total_event[0].total_event);
        if (res.data.total_event[0].total_event < 9) {
          this.setState({ showPagination: false });
        } else {
          this.setState({ showPagination: true });
        }
        this.setState({ events: res.data.events, limit: res.data.limit ,categories: res.data.categories, upcomings: res.data.upcoming , loading: false ,category: res.data.category});
            ...
            <div className="row">
              <div className="col-lg-12">
                <div className="siteTitle">
                  <h2 className="titleText">{this.state.category}</h2>
